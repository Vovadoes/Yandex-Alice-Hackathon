...
def main(event, context):
    ...
    flag = False
    res = get_user(driver, user_id)  # ответ от бд список словарей которые подходят
    print(f"{res=}")
    if len(res) != 0:
        user: dict = res[0]
...
def handle_dialog(req, res, sessionStorage):
    global dict_treasures
    global dict_doors_for_hero
    ...
    if sessionStorage[user_id]['epoch'][0] == '1':
        if sessionStorage[user_id]['epoch'] == '11':  # знает ли правила
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:
            ...
            elif req['request']['original_utterance'].lower() in ['нет', 'не понял', 'не хочу', 'не надо', 'стоп']:
                ...
                res['response']['text'] = f'Тогда давай я расскажу тебе правила:' + pravila + 'Вы поняли правила?'
                res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                              {'title': 'Нет', 'hide': True}]
        ...
        if sessionStorage[user_id]['epoch'] == '2':  # какие карты он достает
            ...
            if 'никакие' in text_res:  # ничего не хочет выкладывать
                ...
                res['response']['text'] += show_names(user_id, sessionStorage)
                res['response']['text'] += '--------->\n'
                # доп вопрос
            ...
            if len(num) == 0:
                ...
                res['response']['text'] += 'Какие карты вы хотите положить на стол? (номера)'
                res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                res['response']["tts"] = 'В вашем ответе отсутствуют цифры! Введите цифры! Какие карты вы хотите положить на стол, номера.'
                return
            res['response'][
            ...
            for x in num:
                if int(x) <= 0 or int(x) > len(sessionStorage[user_id]['cards_on_hands']):
                    ...
                    res['response']['text'] += 'Какие карты вы хотите положить на стол? (номера)'
                    res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                    res['response']["tts"] = f'Ошибка! Вы должны говорить цифры в диапазоне от 1 до {len(sessionStorage[user_id]["cards_on_hands"])}.' \
            ...
            if t:
                ...
                res['response']['text'] += show_names(user_id, sessionStorage)
                res['response']['text'] += '--------->\n'
                k, text = find_free_cards(user_id, sessionStorage)
                if k == 0:
                    # доп вопрос
                    res['response']['text'] += 'Вы готовы открыть дверь?\n'
                    res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                    ...
                    res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                  {'title': 'Нет', 'hide': True}]
                    res['response']["tts"] = 'У вас есть карты, которые можно положить! Хотите это сделать?'
            ...
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:
                ...
                res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                              {'title': 'Нет', 'hide': True}]
                res['response']["tts"] = 'Вы отказались выкладывать карты, хорошо, вы готовы открывать дверь?'
                sessionStorage[user_id]['epoch'] = '22'
            else:  # не поняли
                res['response']['text'] = 'Я вас не поняла, извините... Так да или нет?'
                res['response']['buttons'] = [{'title': 'Да', 'hide': True},
            ...
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:  # ОТКРЫВАЕМ ДВЕРЬ
                ...
                if class_card == 1:  # пользователю попался монстр
                    ...
                    res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                  {'title': 'Нет', 'hide': True}]
                    res['response']["tts"] = 'Вы стучитесь в дверь, и вам выпало проклятье. Вы готовы продолжать?'
            ...
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:
                ...
                if n == 1:  # может победить
                    ...
                    if len([i for i in sessionStorage[user_id]['cards_on_hands'] if
                        ...
                        what, text = do_bad_things_with_hero(user_id, num, sessionStorage)
                        if what == 1:  # смог убежать
                            res['response']['text'] += text
                            res['response']['text'] += 'Вы готовы продолжать?'
                            res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                          {'title': 'Нет', 'hide': True}]
                            res['response']["tts"] = 'Вы смогли убежать от монстра! Вы готовы продолжать?'
                            sessionStorage[user_id]['epoch'] = '33'
                            ...
                            res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                          {'title': 'Нет', 'hide': True}]
                            res['response']["tts"] = 'Вы умерли - как жалко это признавать. Вы хотите играть дальше?'
            ...
            elif req['request']['original_utterance'].lower() in ['нет', 'не понял', 'не хочу', 'не надо', 'стоп']:  # УБЕГАЕМ
                print('УХОДИМ')
                num = random.randint(1, 6)
                res['response'][
            ...
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:
                ...
                res['response']['end_session'] = True
                res['response']["tts"] = 'Приходите когда будете готовы! До скорых встреч!'
            elif req['request']['original_utterance'].lower() in ['нет', 'не понял', 'не хочу', 'не надо', 'стоп']:  # ОТКРЫВАЕМ ДВЕРЬ
                res['response'][
                ...
                if class_card == 1:  # пользователю попался монстр
                    ...
                    res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                  {'title': 'Нет', 'hide': True}]
                    res['response']["tts"] = 'Вы стучитесь в дверь, и вам выпала раса. Вы готовы продолжать?'
                    sessionStorage[user_id]['epoch'] = '30'
            ...
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:  # выбираем бонусы
                ...
                if sessionStorage[user_id]["overall_strength"] + sessionStorage[user_id]["level"] + \
                    ...
                    if what == 1:  # смог убежать
                        ...
                        res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                      {'title': 'Нет', 'hide': True}]
                        res['response']["tts"] = 'Вы не смогли убежать, но вы выжили! Вы готовы продолжать?'
                    else:  # Умер
                        res['response']['text'] += text
                        res['response'][
                            'text'] += 'Вы умерли - как жалко это признавать. Вы хотите играть дальше?'
            ...
            if len(num) == 0:
                ...
                res['response']['text'] += 'Какие бонусы вы хотите выбрать? (номера)'
                res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                res['response']["tts"] = 'В вашем ответе отсутствуют цифры! Какие бонусы вы хотите выбрать? номера'
                return
            res['response'][
                'text'] = f'Вы выбрали позиции: {", ".join(num)}\n'
            for x in num:
                if int(x) <= 0 or int(x) > len(
                    ...
                    res['response']['text'] += 'Какие бонусы вы хотите выбрать? (номера)'
                    res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                    res['response']["tts"] = f"Ошибка! Вы должны говорить цифры в диапазоне от 1 до {len([i for i in sessionStorage[user_id]['cards_on_hands'] if i.__class__.__bases__[0].__name__ == 'BonusBase'])} " + 'Какие бонусы вы хотите выбрать? номера'
                    return
            dict_bonus = {}
            k = 1
            ...
            if sessionStorage[user_id]['monster'] is None:
                ...
                if len([i for i in sessionStorage[user_id]['cards_on_hands'] if
                    ...
                    res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                  {'title': 'Нет', 'hide': True}]
                    res['response']["tts"] = f'Ваши бонусы приняты! Ваша сила: {sessionStorage[user_id]["overall_strength"] + sessionStorage[user_id]["level"] + sessionStorage[user_id]["bonus_strength"]}\nСила монстра: {sessionStorage[user_id]["monster"].level} ' + 'Вы хотите положить еще бонусы?'
                    ...
                    if sessionStorage[user_id]["overall_strength"] + sessionStorage[user_id][
                        ...
                        if what == 1:  # смог убежать
                            ...
                            res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                          {'title': 'Нет', 'hide': True}]
                            res['response']["tts"] = f'У вас нет бонусов, которые могут вам помочь, вам придётся убегать! Подбрасываю кубик и вам выпало {num}. ' + 'Вы смогли убежать! ' + 'Вы готовы продолжать?'
                        elif what == 2:  # выжил
                            res['response']['text'] += text
                            res['response'][
                                'text'] += 'Вы выжили после этого!? Это удивительно!! Идем дальше! '
                            ...
                            res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                          {'title': 'Нет', 'hide': True}]
                            res['response']["tts"] = f'У вас нет бонусов, которые могут вам помочь, вам придётся убегать! Подбрасываю кубик и вам выпало {num}. ' + 'Вы не смогли убежать, но выжили! ' + 'Вы готовы продолжать?'
                        else:  # Умер
                            res['response']['text'] += text
                            res['response'][
                                'text'] += 'Вы умерли - как жалко это признавать. Вы хотите играть дальше?'
                            res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                          {'title': 'Нет', 'hide': True}]
                            res['response']["tts"] = res['response']["tts"] = f'У вас нет бонусов, которые могут вам помочь, вам придётся убегать! Подбрасываю кубик и вам выпало {num}. ' + 'Вы умерли! ' + 'Вы хотите играть дальше?'
                            sessionStorage[user_id]['epoch'] = '213'  # продаем карты
            ...
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:
                ...
                res['response']['text'] += 'Какие карты вы хотите положить на стол? (номера)'
                res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                res['response']["tts"] = 'Вас оживил проходящий волшебник! Вы потеряли вещи но сохранили уровень! ' + 'Какие карты вы хотите положить на стол? номера.'
                sessionStorage[user_id]['epoch'] = '2'  # начинаем игру
            elif req['request']['original_utterance'].lower() in ['нет', 'не понял', 'не хочу', 'не надо', 'стоп']:
                res['response']['text'] = 'Приходите когда будете готовы! До скорых встреч!'
                res['response']['end_session'] = True
        ...
        if sessionStorage[user_id]['epoch'] == '30':
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:
                ...
                if any([True for i in sessionStorage[user_id]['cards_on_hands'] if
                    ...
                    res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                  {'title': 'Нет', 'hide': True}]
                    res['response']["tts"] = 'У вас есть монстры на руках!!! Хотите с ними сразиться?'
                else:
            ...
            if len(num) == 1:
                ...
                choose_monster(user_id, res, sessionStorage)
                res['response']['buttons'] = [{'title': 'Никакого', 'hide': True}]
                res['response']["tts"] = 'Вы ввели цифр больше чем одна! Какого монстра вы хотите сразить?'
            ...
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:
                ...
                if sessionStorage[user_id]['level'] >= 10:
                    ...
                    if len(sessionStorage[user_id]['cards_on_hands']) > 5:
                        ...
                        res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                      {'title': 'Нет', 'hide': True}]
                        res['response']["tts"] = 'У вас больше 5 карт на руках. Вы хотите убрать карту со стола?'
                        ...
                        sessionStorage[user_id]['epoch'] = '42'  # доп правила
            elif req['request']['original_utterance'].lower() in ['нет', 'не понял', 'не хочу', 'не надо', 'стоп']:  # выход из игры
                res['response']['text'] = 'Приходите когда будете готовы! До скорых встреч!'
                res['response']['end_session'] = True
                res['response']["tts"] = 'Приходите когда будете готовы! До скорых встреч!'
            else:  # не поняли
        ...
        if sessionStorage[user_id]['epoch'] == '42':  # готов продолжать + новый цикл
            if req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:
                ...
                if len(main_list) != 0:
                    ...
                    res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                  {'title': 'Нет', 'hide': True}]
                    res['response']["tts"] = f'На вашем столе нет карт! У вас {len(sessionStorage[user_id]["cards_on_hands"])} карт на руке. Вы хотите положить карты на стол?'
                ...
                res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                              {'title': 'Нет', 'hide': True}]
                res['response']["tts"] = f'У вас {len(sessionStorage[user_id]["cards_on_hands"])} карт. Вы хотите положить карты на стол?'
                sessionStorage[user_id]['epoch'] = '43'
            else:
                res['response']['text'] = 'Я вас не поняла, извините... Так да или нет?'
                res['response']['buttons'] = [{'title': 'Да', 'hide': True},
            ...
            if len(num) == 0:
                ...
                res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                res['response']["tts"] = 'В вашем ответе отсутствуют цифры! Какие карты вы хотите убрать со стола в руки?'
                return
            res['response'][
                'text'] = f'Вы выбрали позиции: {", ".join(num)}\n'
            for x in num:
                if int(x) <= 0 or int(x) > len(main_list):
                    ...
                    for i in range(len(main_list)):
                        res['response'][
                            'text'] += f'{i + 1}. {main_list[i][0].title} - {main_list[i][1]}\n'
                    res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                    res['response']["tts"] = f'Ошибка! Вы должны говорить цифры в диапазоне от 1 до {len(main_list)} Какие карты вы хотите убрать со стола в руки?'
                    return
            ml = []
            ...
            if req['request']['original_utterance'].lower() in ['нет', 'не понял', 'не хочу', 'не надо', 'стоп']:
                res['response']['text'] = 'Приходите когда будете готовы! До скорых встреч!'
                res['response']['end_session'] = True
                res['response']["tts"] = 'Приходите когда будете готовы! До скорых встреч!'
            elif req['request']['original_utterance'].lower() in ['да', 'хорошо', 'ок', 'ладно', 'понятно', 'дальше']:
            ...
            if 'никакие' in gg:  # ничего не хочет продавать
                ...
                if len(sessionStorage[user_id]['cards_on_hands']) >= 6:
                    ...
                    res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                                  {'title': 'Нет', 'hide': True}]
                    res['response']["tts"] = 'Вы прошли круг, молодец! Вы остались живы! Давайте открывать еще двери! Вы готовы продолжать?'
                ...
                if len(num) == 0:
                    ...
                    sessionStorage[user_id]['epoch'] = '44'
                    res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                    res['response']["tts"] = 'В вашем ответе отсутствуют цифры! Какие карты вы хотите скинуть или продать?'
                    return
                res['response'][
                    'text'] = f'Вы выбрали позиции: {", ".join(num)}\n'
                for x in num:
                    if int(x) <= 0 or int(x) > len(sessionStorage[user_id]['cards_on_hands']):
                        ...
                        sessionStorage[user_id]['epoch'] = '44'
                        res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                        res['response']["tts"] = f'Ошибка! Вы должны говорить цифры в диапазоне от 1 до {len(sessionStorage[user_id]["cards_on_hands"])} Какие карты вы хотите скинуть или продать?'
                        return
                ...
                for i in range(len(sessionStorage[user_id]['cards_on_hands'])):
                    try:
                        ...
                        res['response'][
                            'text'] += f'{i + 1}) {sessionStorage[user_id]["cards_on_hands"][i].title} (0 $)\n'
                res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                res['response']["tts"] = 'Вы отказались класть карты! Какие карты вы хотите скинуть или продать?'
                sessionStorage[user_id]['epoch'] = '44'
                return
            else:
                # проверка на правильность
                    ...
                    for i in range(len(sessionStorage[user_id]['cards_on_hands'])):
                        try:
                            ...
                            res['response'][
                                'text'] += f'{i + 1}) {sessionStorage[user_id]["cards_on_hands"][i].title} (0 $)\n'
                    res['response']['buttons'] = [{'title': 'Никакие', 'hide': True}]
                    res['response']["tts"] = 'Ваши действия применились! Какие карты вы хотите скинуть или продать?'
                    sessionStorage[user_id]['epoch'] = '44'
                    return
                else:
                    res['response']['text'] += 'Ошибка!\n' + cards
...
def catch_door(user_id, res, sessionStorage):
    global dict_doors_for_hero
    res['response']['card'] = {}
    res['response']['card']['type'] = 'BigImage'
    res['response']['card']['title'] = 'Вы готовы продолжать?'
...
def del_all_things(user_id, sessionStorage):
...
def do_bad_things_with_hero(user_id, num, sessionStorage):
    text = ''
...
def choose_bonus(user_id, sessionStorage):
...
def catch_bonus(user_id, res, sessionStorage):  # текст для вывода награды
    global dict_treasures
    text = ''
...
def fight_with_monster(user_id, res, sessionStorage):
    ...
    if monstr.level < sessionStorage[user_id]['overall_strength'] + sessionStorage[user_id][
        ...
        res['response']['buttons'] = [{'title': 'Да', 'hide': True},
                                      {'title': 'Нет', 'hide': True}]
        res['response']["tts"] = 'У вас не хватает силы, чтобы победить его! Вы хотите использовать бонусы?'
        sessionStorage[user_id]['epoch'] = '25'
        return 2


def is_all_right(user_id, nums, sessionStorage):  # проверяем на правильность выбора карт
...
def find_free_cards(user_id, sessionStorage):  # передаю num от 1 - бес
...
def sort_cards(user_id, sessionStorage):  # сортировка карт в руке
    list_obj = sessionStorage[user_id]['cards_on_hands']
    s = []
    for i in list_obj:
...
def show_names(user_id, sessionStorage):  # показ амундирования
    dd = {1: 'Эльф', 2: 'Хафлинг', 3: 'Дварф'}
...
def show_not_all_cards(user_id, res, sessionStorage):
    dd = {1: 'Эльф', 2: 'Хафлинг', 3: 'Дварф'}
    armor = sessionStorage[user_id]['armor']
    weapon = sessionStorage[user_id]['weapon']
    class_human = sessionStorage[user_id]['class']
